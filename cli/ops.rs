...
fn op_dl_call(
  ...
  let result =
    resources::call_dl_op(rid, is_sync, inner.data().unwrap(), data)?;

  let map_response = move |buf: Buf| {
    let builder = &mut FlatBufferBuilder::new();
  ...
  match (is_sync, result) {
    (true, Op::Sync(buf)) => {
      let response_buf = map_response(buf)?;
      Ok(Op::Sync(response_buf))
    }
    (false, Op::Async(future)) => Ok(Op::Async(Box::new(
      future.map_err(DenoError::from).and_then(map_response),
    ))),
    (is_sync, _) => panic!(
